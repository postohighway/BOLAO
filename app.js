(function () {
  ...
  function ensureDbShape() {
    ...
    db.unidades = db.unidades || ['Unidade Matriz', 'Unidade Zona Sul', 'Unidade Centro'];
    db.despesas = db.despesas || [];
    if (!db.funcionarios.some((f) => f.id === 'fn-juliano')) {
      db.funcionarios.push({
        id: 'fn-juliano',
        nome: 'Juliano',
        cargo: 'Apuração de despesas (lanches / refeições)',
        observacoes: 'As despesas com status Enviado são apuradas neste cadastro.',
        ativo: true,
      });
    }
  }
  ...
  function renderDashboard() {
    ...
    const julId = 'fn-juliano';
    const despesas = db.despesas || [];
    const hojeD = new Date();
    const despJulMes = despesas
      .filter(
        (d) =>
          d.funcionarioId === julId &&
          d.status === 'enviado' &&
          d.data &&
          (() => {
            const dt = new Date(d.data);
            return dt.getMonth() === hojeD.getMonth() && dt.getFullYear() === hojeD.getFullYear();
          })()
      )
      .reduce((s, d) => s + (d.valor || 0), 0);
    items.push(['🍽️', 'Despesas Juliano (mês)', fmtMoney(despJulMes)]);
  ...
  // ---------- Despesas (lanches / refeições) + apuração Juliano ----------
  const JULIANO_ID = 'fn-juliano';

  function despesasJulianoFiltradas() {
    return (db.despesas || []).filter((d) => d.funcionarioId === JULIANO_ID && d.status === 'enviado');
  }

  function renderBlocoJuliano() {
    const box = document.getElementById('juliano-apuracao');
    if (!box) return;
    const jul = (db.funcionarios || []).find((f) => f.id === JULIANO_ID);
    const lista = despesasJulianoFiltradas();
    const totalGeral = lista.reduce((s, d) => s + (d.valor || 0), 0);
    const hoje = new Date();
    const totalMes = lista
      ...
      .reduce((s, d) => s + (d.valor || 0), 0);
    const rascunho = (db.despesas || []).filter((d) => d.funcionarioId === JULIANO_ID && d.status === 'rascunho');
    const totalRascunho = rascunho.reduce((s, d) => s + (d.valor || 0), 0);

    const porUnidade = {};
    lista.forEach((d) => {
      const u = d.unidade || '—';
      porUnidade[u] = (porUnidade[u] || 0) + (d.valor || 0);
    });
    const linhasUn = Object.keys(porUnidade)
      .sort()
      .map((u) => `<tr><td>${u}</td><td><strong>${fmtMoney(porUnidade[u])}</strong></td></tr>`)
      .join('');

    box.innerHTML = `
      <div class="juliano-card">
        <h3>👤 Cadastro — ${jul ? jul.nome : 'Juliano'}</h3>
        <p class="juliano-sub">${jul ? jul.cargo : ''}${jul && jul.observacoes ? ' · ' + jul.observacoes : ''}</p>
        <div class="juliano-stats">
          <div class="juliano-stat"><span class="juliano-lbl">Total apurado (enviadas)</span><span class="juliano-val">${fmtMoney(totalGeral)}</span></div>
          <div class="juliano-stat"><span class="juliano-lbl">Total mês atual</span><span class="juliano-val">${fmtMoney(totalMes)}</span></div>
          <div class="juliano-stat warn"><span class="juliano-lbl">Em rascunho (não apuradas)</span><span class="juliano-val">${fmtMoney(totalRascunho)}</span></div>
        </div>
        <p class="hint" style="margin-top:0.75rem">Somente despesas com status <strong>Enviado</strong> entram na apuração do Juliano. Use &quot;Enviar&quot; na lista abaixo.</p>
        ${linhasUn ? `<div class="table-wrap" style="margin-top:1rem"><table class="data"><thead><tr><th>Unidade</th><th>Total enviado</th></tr></thead><tbody>${linhasUn}</tbody></table></div>` : ''}
      </div>`;
  }

  function renderDespesas() {
    renderBlocoJuliano();
    const tb = document.querySelector('#tbl-despesas tbody');
    const catLabel = { lanches: 'Lanches', refeicoes: 'Refeições', outros: 'Outros' };
    tb.innerHTML = (db.despesas || [])
      .map(
        (x) => `
      <tr>
        <td>${fmtDate(x.data)}</td>
        <td>${x.unidade || '—'}</td>
        <td>${catLabel[x.categoria] || x.categoria}</td>
        <td>${fmtMoney(x.valor)}</td>
        <td>${(db.funcionarios || []).find((f) => f.id === x.funcionarioId)?.nome || x.funcionarioId}</td>
        <td><span class="badge ${x.status === 'enviado' ? 'badge-ok' : 'badge-warn'}">${x.status === 'enviado' ? 'Enviado' : 'Rascunho'}</span></td>
        <td class="actions">
          ${x.status === 'rascunho' ? `<button class="btn btn-sm btn-primary" data-enviar-d="${x.id}">Enviar p/ apuração</button>` : ''}
          <button class="btn btn-sm btn-secondary" data-edit-d="${x.id}">Editar</button>
          <button class="btn btn-sm btn-danger" data-del-d="${x.id}">Excluir</button>
        </td>
      </tr>`
      )
      .join('');
    tb.querySelectorAll('[data-enviar-d]').forEach((b) =>
      b.addEventListener('click', () => {
        const d = db.despesas.find((x) => x.id === b.dataset.enviarD);
        if (d) {
          d.status = 'enviado';
          persist();
        }
    ...
    tb.querySelectorAll('[data-edit-d]').forEach((b) => b.addEventListener('click', () => editDespesa(b.dataset.editD)));
    tb.querySelectorAll('[data-del-d]').forEach((b) => b.addEventListener('click', () => delDespesa(b.dataset.delD)));
  }

  document.getElementById('btn-nova-despesa').addEventListener('click', () => editDespesa(null));

  function editDespesa(id) {
    const x = id ? db.despesas.find((d) => d.id === id) : {};
    const unOpts = (db.unidades || [])
      .map((u) => `<option value="${u.replace(/"/g, '&quot;')}" ${x.unidade === u ? 'selected' : ''}>${u}</option>`)
      .join('');
    const fnOpts = (db.funcionarios || [])
      .filter((f) => f.ativo !== false)
      .map((f) => `<option value="${f.id}" ${(x.funcionarioId || JULIANO_ID) === f.id ? 'selected' : ''}>${f.nome}</option>`)
      .join('');
    openModal(id ? 'Editar despesa' : 'Nova despesa', `
      <form id="f-desp">
        <div class="form-group"><label>Data</label><input type="date" name="data" required value="${x.data || new Date().toISOString().slice(0, 10)}" /></div>
        <div class="form-group">
          <label>Unidade (onde ocorreu o gasto)</label>
          <input name="unidade" list="lista-unidades" required value="${x.unidade || ''}" placeholder="Ex.: Unidade Matriz" />
          <datalist id="lista-unidades">${(db.unidades || []).map((u) => `<option value="${u.replace(/"/g, '&quot;')}"></option>`).join('')}</datalist>
        </div>
        <div class="form-row">
          <div class="form-group">
            <label>Categoria</label>
            <select name="categoria">
              <option value="lanches" ${x.categoria === 'lanches' ? 'selected' : ''}>Lanches</option>
              <option value="refeicoes" ${x.categoria === 'refeicoes' ? 'selected' : ''}>Refeições</option>
              <option value="outros" ${x.categoria === 'outros' ? 'selected' : ''}>Outros</option>
            </select>
          </div>
          <div class="form-group"><label>Valor (R$)</label><input type="number" step="0.01" name="valor" required value="${x.valor || 0}" /></div>
        </div>
        <div class="form-group"><label>Descrição</label><input name="descricao" value="${(x.descricao || '').replace(/"/g, '&quot;')}" placeholder="Ex.: Lanche equipe manhã" /></div>
        <div class="form-group">
          <label>Apurar no cadastro de</label>
          <select name="funcionarioId">${fnOpts}</select>
        </div>
        <div class="form-group">
          <label>Status</label>
          <select name="status">
            <option value="rascunho" ${x.status !== 'enviado' ? 'selected' : ''}>Rascunho (ainda não apura no Juliano)</option>
            <option value="enviado" ${x.status === 'enviado' ? 'selected' : ''}>Enviado (entra na apuração)</option>
          </select>
        </div>
        <div class="form-actions">
          <button type="button" class="btn btn-secondary" data-close>Cancelar</button>
    ...
    modalBody.querySelector('#f-desp').addEventListener('submit', (e) => {
      e.preventDefault();
      const fd = new FormData(e.target);
      const o = {
        data: fd.get('data'),
        ...
        valor: +fd.get('valor'),
        descricao: fd.get('descricao'),
        funcionarioId: fd.get('funcionarioId') || JULIANO_ID,
        status: fd.get('status'),
      };
      if (id) {
        const i = db.despesas.findIndex((d) => d.id === id);
        db.despesas[i] = { ...db.despesas[i], ...o };
      } else {
        o.id = uid('dsp');
        db.despesas.push(o);
      }
      const u = o.unidade.trim();
